...
mutation 
editFriend($friend : FriendEditInput){
...
const AddFriend = ({ initialFriend, allowEdit }: AddFriendProps) => {
  ...
  const [editFriend, { data: data2 }] = useMutation(
    EDIT_FRIEND,
    {
      update(cache, {data}) {
        ...
        let allFriends = d.getAllFriends || []
        cache.writeQuery({ query: ALL_FRIENDS, data: { getAllFriends: [...allFriends, editedFriend] } })
      }
    }
  )
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (allowEdit) {
      editFriend({
        variables: {
          friend: { firstName: friend.firstName, lastName: friend.lastName, email: friend.email, password: friend.password}
        }
      })
     } else {
    addFriend({
